...
export function buildPhase2SystemPrompt(row: ProjectRow, repoRoot: string): string {
  ...
  return [
    ...
    "11. Phase 1 stores each generated page source in downloads/{projectId}/{pageName}/{pageName}.html (with index.html alongside it). Treat these files as the authoritative generated HTML source for Phase 2.",
    "12. When the implementation task requires converting an exported HTML page to PHP/dynamic PHP, first read the complete corresponding .html file from the repository. Then write the final standalone PHP result to downloads/{projectId}/{pageName}/{pageName}.php (or the exact PHP output location required by the verified project structure). Do not overwrite the original HTML source.",
    "13. After creating or updating a PHP file, verify its contents and update the existing PHP-related output sheet column if one exists in the live headers (for example 'PHP File'). Use the exact live column header and record a direct GitHub Pages download URL using the same project/page path convention. If no PHP output column exists, do not invent a new schema silently; record the result in AI Engine Note unless the task explicitly asks to create a column.",
    "14. For HTML/PHP implementation work, inspect the entire source file before editing. Do not truncate the source or reason from a partial excerpt when the task depends on the full page.",
    "",
